  ...
  return (
    <Layout scroll header={<AppHeader />} footer={<AppFooter />} maxWidth={null}>
      ...
      <View style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 1200, alignSelf: "center" }}>
        {/* HERO — affiché uniquement si NON connecté */}
        {!isLogged && (
          <View style={{ backgroundColor: "#121212", borderWidth: 1, borderColor: BORDER, borderRadius: 18, padding: 18, marginBottom: 18 }}>
            ...
            <Text style={{ color: TEXT, fontSize: 22, fontWeight: "800", marginBottom: 6 }}>
              Gérez vos abonnements en famille sans prise de tête
            </Text>
            <Text style={{ color: MUTED, fontSize: 14 }}>
              Suivez les paiements, évitez les doublons et gardez la main sur le budget.
            </Text>
          </View>
        )}
        ...
        {/* QUICK ACTIONS + (Pourquoi nous ?) — la colonne droite n’apparaît que si NON connecté */}
        <View
          style={{
            gap: !isLogged && isWide ? 140 : 0,
            marginBottom: 18,
            ...(isWide && !isLogged ? { flexDirection: "row" } : {}),
          }}
        >
            ...
            <View style={{ backgroundColor: CARD, borderRadius: 16, borderWidth: 1, borderColor: BORDER, padding: 14 }}>
              ...
              {!isLogged ? (
                ...
                <>
                  ...
                  <ActionTile icon="mail-unread-outline" label="Mes invitations" onPress={() => nav.navigate("MyInvitations")} />
                  <ActionTile icon="log-out-outline" label="Se déconnecter" onPress={logout} />
                </>
              )}
            </View>
          </View>

          {/* Colonne droite : “Pourquoi nous ?” — MASQUÉE si connecté */}
          {!isLogged && (
            <View style={{ flex: 1 }}>
              <View style={{ backgroundColor: CARD, borderRadius: 16, borderWidth: 1, borderColor: BORDER, padding: 14 }}>
                ...
                {[
                  ...
                  "Création d'espaces (famille, colocs, équipe)",
