...
import {
  View, Text, TouchableOpacity, TextInput, SafeAreaView,
...
const ABS = (p) =>
  !p ? null : p.startsWith("http")
    ? p
    : `${(process.env.EXPO_PUBLIC_API_BASE_URL || "").replace(/\/$/, "")}/${String(p).replace(/^\//, "")}`;
...
export default function ProfileScreen() {
  ...
  const avatarUri = useMemo(() => ABS(user?.avatar || user?.avatar_url), [user]);
  const [imgOk, setImgOk] = useState(Boolean(avatarUri));
  useEffect(() => { setImgOk(Boolean(avatarUri)); }, [avatarUri]);
              ...
              <View style={styles.page}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 12 }}>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    <Text style={styles.nameLine}>
                      {[firstname, lastname].filter(Boolean).join(" ") || username || "Utilisateur"}
                    </Text>
                    <Text style={styles.muted}>{email || "—"}</Text>
                  </View>

                  {!edit && (
                    <TouchableOpacity onPress={() => setEdit(true)} style={[styles.btn, styles.btnGhost]}>
                ...
                <View style={styles.card}>
                  ...
                  <Text style={styles.muted}>
                    {Array.isArray(user?.roles) ? user.roles.join(", ") : user?.role || "ROLE_USER"}
                  </Text>
                </View>
